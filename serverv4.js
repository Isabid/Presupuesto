...
app.post('/api/items', async (req, res) => {
  // tomar el codigo por separado para hacer la vaidacion de si existe o no.
  ...
  const { codigo, nombre, enero, febrero, marzo, abril, mayo, junio, julio, agosto, septiembre, octubre, noviembre, diciembre, totalano, ano, r1, r2, r3, r4, r5, r6, r7, r8, r9, r10, r11, r12, r13} = req.body;
   ...
   try {
    ...
    // para depurar
    console.log('Código desde el cliente:', codigoDesdeCliente);
    ...
    if (existingItem.rows.length) {
      ...
      console.log("Aqui entre cuando encuentra el registo", codigoDesdeCliente, nombre, codigo)
        // Ya existe un registro con el mismo código, devolver un mensaje de error
        //return res.status(400).json({ error: 'Ya existe un registro con el mismo código, desde Servidor' });
      return res.status(200).json({ success: false, error: 'Ya existe un registro con el mismo código, desde Servidor' });
    }
