...
function enviarFormulario() {
  ...
  .then(data => {
    if (data.success) {
      // Operación exitosa
      console.log("Trae mensaje del servidor", data.mensaje);
  ...
  setTimeout(function() {
    // reinicia app para que los datos queden actualizados
      //reiniciarApp();
  }, 2000); // El tiempo está especificado en milisegundos (2 segundos en este caso)
}
...
const reiniciarServidor = async () => {
  try {
    ...
    if (response.ok) {
      ...
      console.log('Respuesta del servidor al reiniciar:', data);
    } else {
